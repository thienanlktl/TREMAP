/**
 * Pure Simpson IFC bearing parser — NO Node `fs`/`path`.
 * Parses a Simpson IFC export for per-truss bearing Connection Type
 * (Wall / Hanger-To-Truss). Used by the Node build and the browser runtime.
 */
import { normalizeTrussMark } from "./truss-links.js";

export function parseSimpsonIfcBearingsText(text, sourceName = "ifc") {
  ...
  return { byMark, source: sourceName, found: true };
}
